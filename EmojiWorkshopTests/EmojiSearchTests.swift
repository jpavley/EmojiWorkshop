...
class EmojiSearchTests: XCTestCase {
    ...
    func testCantFindEmojiFile() {
        guard let testEmojiCollection = EmojiCollection(sourceFileName: Identifiers.emojiTestFail) else {
            return
        }
        
        XCTAssertNotNil(testEmojiCollection)
    }
